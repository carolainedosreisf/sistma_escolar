...
<div class="modal-fora">
    <div class="modal-dentro">
        ...
        <form action="" method="post">
            <button name="botao_ok" class="botao-ok">Ok</button>
        </form>
 ...
 <?php if(isset($_POST['cadastra'])){
    ...
    $sql = "INSERT INTO cursos (curso) VALUES ('$curso')";	
        ...
        if ($curso !== ''){
            ...
            if ($cad == ''){
                echo "<script language='javascript'> modal('flex'); modal_texto('Curso já cadastrado'); </script>";
            }else{
                echo "<script language='javascript'> 
                        modal('flex'); modal_texto('Cadastro Realizado com sucesso!'); 
                    </script>"; 
                 
                                   
            }
        }else{
            $url = 'cursos_e_disciplinas.php?pg=curso';
            
            echo "<script language='javascript'> modal('flex'); modal_texto('Campo vazio!'); </script>";  
        }
       

    }?> 
<?php 
 if(isset($_POST['botao_ok'])){
    echo $url;
     if ($url != ''){
      ...
      <?php while($res_1 = mysqli_fetch_assoc($result)){ ?>
        <tr>
            <td><h3><?php echo $curso = $res_1['curso']; ?></h3></td>
                <td>
                    <h3><?php $sql_2 = "SELECT * FROM disciplinas WHERE curso = '$curso'";
                    $result2 = mysqli_query($conexao, $sql_2);
                    echo mysqli_num_rows($result2); ?></h3>
                </td>
            <td>
                <a class="a" href="cursos_e_disciplinas.php?pg=curso&deleta=cur&id=<?php echo @$res_1['id']; ?>"><img title="Excluir curso" src="img/deleta.jpg" width="18" height="18" border="0"></a>
            </td>
        </tr>
      <?php } ?>
    </table>	 
...
<?php if(@$_GET['pg'] == 'disciplina'){ ?>
    ...
    <?php if(isset($_POST['cadastra'])){
        ...
        $turno = $_POST['turno'];	
        if($disciplina == ''){
            echo "<script language='javascript'> modal('flex'); modal_texto('Digite o nome da disciplina!'); </script>";
        }else if($sala == ''){
            echo "<script language='javascript'> modal('flex'); modal_texto('Digite o n° da sala!'); </script>";
        }else{
            ...
            if($cad_disc == ''){
                ...
                echo "<script language='javascript'> modal('flex'); modal_texto('Disciplina cadastrada com sucesso!'); </script>";
                echo "<script>setTimeout(function(){ window.location='cursos_e_disciplinas.php?pg=disciplina'; }, 3000);</script> ";
    ...
    <form name="form1" method="post" action="">
        <table width="900" border="0">
            ...
            <tr>
                ...
                <td>
                    <input type="text" name="sala" id="textfield">
                </td>
                <td>
                ...
                <td width="126">
                    <input class="input" type="submit" name="cadastra" id="button" value="Cadastrar">
                </td>
            </tr>    
        </table>
    </form>
